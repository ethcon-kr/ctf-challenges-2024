...
@auth.route("/graylab", methods=["POST", "GET"])
@ratelimit(method="POST", limit=10, interval=5)
def id_login():
    errors = get_errors()
    if request.method == "POST":
...
@auth.route("/connect", methods=["POST", "GET"])
@ratelimit(method="POST", limit=10, interval=5)
def login():
    if current_user.authed():
        return jsonify({"status": "fail", "next": url_for("challenges.listing")})

    if request.method == "POST":
        siwe = SiweMessage.from_message(request.form["name"])
        signature = request.form["password"]

        user = Users.query.filter_by(email=siwe.address).first()
        if user:
            if user and siwe.verify(signature=signature) == None:
                session.regenerate()
                login_user(user)
                log("logins", "[{date}] {ip} - {name} logged in", name=user.name)
                print(session)
                ...
                next_url = (
                    ...
                    and validators.is_safe_url(request.args.get("next"))
                    else url_for("challenges.listing")
                )
                return jsonify({"status": "success", "next": next_url})
            ...
            with app.app_context():
                ...
                if request.args.get("next") and validators.is_safe_url(
                    ...
                    return jsonify(
                        {"status": "success", "next": url_for("challenges.listing")}
                    )
    else:
