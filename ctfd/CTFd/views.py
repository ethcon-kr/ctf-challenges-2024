...
@views.route("/raffle-event", methods=["GET"])
def raffle():
    Model = get_model()
    solvers = Model.query.join(Solves, Solves.account_id == Model.id).filter(Solves.challenge_id == 1)

    addrs = []
    for solver in filter(lambda x: x.email[:2] == "0x", solvers):
        addrs.append(solver.email)

    # notifications = Notifications.query.order_by(Notifications.id.desc()).all()
    #users = Users.query.filter_by(banned=False, hidden=False)
    #    .filter(*filters)
    #    .order_by(Users.id.asc())
    return render_template("raffle-event.html", solvers=addrs)
