...
def get_all_challenges(admin=False, field=None, q=None, **query_args):
    ...
    for c in chal_q:
        ...
        results.append(ct)
    newResults = []
    newResults.extend(list(filter(lambda x: x.category == "investigation", results)))
    newResults.extend(list(filter(lambda x: x.category == "intermediate", results)))
    newResults.extend(list(filter(lambda x: x.category == "beginner", results)))
    return newResults
